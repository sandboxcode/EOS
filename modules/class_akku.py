...
class PVAkku:
    def __init__(self, kapazitaet_wh=None, hours=None, lade_effizienz=0.88, entlade_effizienz=0.88,
        ...
        # Set initial state of charge (SoC) using the new method
        self.set_soc_prozent(start_soc_prozent)
    ...
    def set_soc_prozent(self, soc_prozent):
        """
        Set the state of charge (SoC) as a percentage, considering min and max limits.
        
        Parameters:
        soc_prozent (float): State of charge in percentage (0-100)
        """
        # Calculate soc in Wh based on percentage and apply limits
        soc_wh = (soc_prozent / 100) * self.kapazitaet_wh
        # Ensure soc_wh is within min and max SoC limits
        self.soc_wh = min(max(soc_wh, self.min_soc_wh), self.max_soc_wh)
    ...
    def ladezustand_in_prozent(self):
        """
        Calculate the state of charge (SoC) as a percentage of the total usable capacity.
        This method now considers the usable energy content based on the current state of charge.
        """
        # Calculate the current usable energy in Wh, considering the efficiency losses and min SoC.
        #nutzbare_energie = self.aktueller_energieinhalt()  # Use the current usable energy
        # Calculate the percentage of usable energy relative to the total battery capacity.
...
if __name__ == '__main__':
    ...
    akku = PVAkku(kapazitaet_wh=10000, hours=1, start_soc_prozent=10, min_soc_prozent=20, max_soc_prozent=80)
    akku.reset()
    ...
    akku.reset()
    print(f"Initial SoC: {akku.ladezustand_in_prozent()}%")    
    abgegeben_wh, verlust_wh = akku.energie_abgeben(10000, 0)    
    
    print(f"SoC : {abgegeben_wh} Wh, Losses: {verlust_wh} Wh")
    print(f"Energy discharged: {abgegeben_wh} Wh, Losses: {verlust_wh} Wh")
    print(f"SoC after discharge: {akku.ladezustand_in_prozent()}%")
